...
int main() {
	...
	//1.socket 생성
	SOCKET cSock;
	cSock = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP); //주소계층(체계), 타입, 구체적인 프로토콜 / PF_INET => 프로토콜 INET, AF_INET => address INET 값은 같음
	...
	while (1) {
		...
		memset(&svrAddr, 0, sizeof(svrAddr));
		svrAddr.sin_family = AF_INET;
		svrAddr.sin_addr.S_un.S_addr = inet_addr(serverIP);//inet_addr을 사용하면 스트링을 32비트 빅엔디언으로 바꿔서 삽입해줌
		//svrAddr.sin_addr.s_addr 로도 32비트 사용가능
		...
		ret = recvfrom(cSock, &result, sizeof(result), 0, (SOCKADDR*)&fromAddr, &size); //UDP는 메시지 단위를 쪼개서 보내도 알아서 붙여서 여러번 받을 필요 X
...
#endif
		//서버로부터 result 수신 완료.
		printf("Client> result from 서버 = %d \n", result); //buf가 char타입이라서 형변환 해줘야함
	}
